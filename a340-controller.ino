...
bool isKickdownRequested() {
  ...
  return (sensors.throttle_percent > KICKDOWN_THRESHOLD) && (millis() - throttle_change_time < 200);
}
...
int calculateLockupDuty() {
  bool can_lockup = (trans_state.current_gear >= LOCKUP_ENABLE_GEAR) && (sensors.vehicle_speed_kmh > LOCKUP_ENABLE_SPEED) && (sensors.throttle_percent < LOCKUP_THROTTLE_MAX) && (trans_state.shift_state == SHIFT_STABLE) && (sensors.fluid_temp_c > 50);

  bool must_unlock = (sensors.vehicle_speed_kmh < LOCKUP_DISABLE_SPEED) || (sensors.throttle_percent > LOCKUP_THROTTLE_MAX + 10) || (trans_state.shift_state != SHIFT_STABLE) || (trans_state.current_gear < LOCKUP_ENABLE_GEAR);
...
void printDiagnostics() {
  ...
  Serial.println(trans_state.current_gear);
  Serial.print("Target Gear: ");
  Serial.println(trans_state.target_gear);
  Serial.print("Lockup: ");
  Serial.println(trans_state.lockup_engaged ? "ENGAGED" : "DISENGAGED");
  Serial.print("Power Mode: ");
  Serial.println(trans_state.power_mode ? "ON" : "OFF");
  Serial.print("Total Shifts: ");
  Serial.println(stats.total_shifts);
  ...
  Serial.println(sensors.engine_rpm);
  Serial.print("Fluid Temp: ");
  Serial.print(sensors.fluid_temp_c);
  Serial.println("°C");
  Serial.print("Max Temp: ");
  Serial.print(stats.max_temp_c);
  Serial.println("°C");

  Serial.println("\n--- Control Outputs ---");
  Serial.print("Accumulator Duty: ");
  Serial.print(trans_state.accumulator_duty);
  Serial.println("%");
  Serial.print("Lockup Duty: ");
  Serial.print(trans_state.lockup_duty);
  Serial.println("%");

  Serial.println("\n--- Adaptive Learning ---");
  Serial.print("1-2 Shift Offset: ");
  Serial.println(trans_state.shift_quality_offset[0]);
  Serial.print("2-3 Shift Offset: ");
  Serial.println(trans_state.shift_quality_offset[1]);
  Serial.print("3-4 Shift Offset: ");
  Serial.println(trans_state.shift_quality_offset[2]);
